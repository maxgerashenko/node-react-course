...
export default function Conditions() {
  ...
  const getNotificationObject = (text: string) => ({
    ...
    return (
      <div>
        ...
        {(() => {
          switch (status) {
            case 'info':
              return <a> info component: {text} </a>;
            case 'warning':
              return <p> warning component: {text} </p>;
            case 'error':
              return <div> error component: {text} </div>;
            default:
              return null;
          }
        })()}
  ...
  function Notification3({ status, text }: { status: Status; text: string }) {
    return <div>{getNotificationObject(text)[status]}</div>;
  }
  ...
  return (
    <div>
      ...
      <p>showUsers 1{showUsers ? List({ users }) : null}</p>
      <p>showUsers 2{showUsers && List({ users })}</p>
      <p>isLoading template 2{LoadingIndicator({ isLoading })}</p>
      <p>Notification {Notification({ ...notificationStatus })}</p>
      <p>Notification2 {Notification2({ status })}</p>
      <p>Notification3 {Notification3({ ...status3 })}</p>
      <p>Message {Message({ ...messageProp })}</p>
    </div>
  );
}
